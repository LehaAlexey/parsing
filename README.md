...
Файл, покрытый тестами - internal/bootstrap/parser/extractor_test.go
...
Он запускает обработку по нескольким ссылкам через парсер.

Запустить его можно через cmd.exe (Windows) в папке `cmd\\pricecheck`:
`go run .`
